...
export default function FiveHintsBuzzPanel({ round, myId, players, onBuzzIn, onSubmitGuess }) {
  ...
  if (isActiveBuzzer) {
    return (
      <div className="hoop-card-outline animate-slide-up mx-auto max-w-lg space-y-4 p-6 text-center">
        <p className="hoop-stat-label text-terracotta">You buzzed in!</p>
  ...
  if (someoneElseBuzzing) {
    return (
      <div className="hoop-card-outline mx-auto max-w-lg space-y-3 p-6 text-center">
        <p className="text-lg text-ink">
          <span className="font-semibold text-terracotta">{buzzerName}</span> is guessing…
        </p>
        <Timer startedAt={round.buzzDeadlineAt - COMPETITIVE_BUZZ_SECONDS * 1000} durationSeconds={COMPETITIVE_BUZZ_SECONDS} />
      </div>
    );
  ...
  if (amLockedOut) {
    return (
      <div className="mx-auto max-w-lg text-center">
        <p className="text-sm text-ink/60">You guessed wrong this round — locked out until the next one.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex justify-center">
      <button
        onClick={onBuzzIn}
        className="hoop-btn-primary animate-fade-in px-10 py-5 text-xl shadow-lg shadow-terracotta/20"
      >
        Buzz In
      </button>
    </div>
